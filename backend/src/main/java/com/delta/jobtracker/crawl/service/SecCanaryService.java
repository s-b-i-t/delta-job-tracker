...
public class SecCanaryService {
  ...
  public SecCanarySummary runSecCanary(
    ...
    DomainResolutionResult domainResolution =
        new DomainResolutionResult(0, 0, 0, 0, 0, 0, List.of());
    ...
    try (CanaryHttpBudgetContext.Scope scope = CanaryHttpBudgetContext.activate(budget)) {
      ...
      if (tickers.isEmpty()) {
        ...
        if (deadlineExceeded(deadline)) {
          ...
          CareersDiscoveryWithMetrics discoveryWithMetrics =
              careersDiscoveryService.discoverForTickersWithMetrics(
                  tickers, tickers.size(), deadline, vendorProbeOnly);
          ...
          careersDiscovery =
              discoveryWithMetrics == null ? careersDiscovery : discoveryWithMetrics.result();
          careersDiscoveryMetrics =
              discoveryWithMetrics == null
                  ? careersDiscoveryMetrics
                  : discoveryWithMetrics.metrics();
  ...
  private void runAndPersistCanary(
      long runId, int requestedLimit, boolean vendorProbeOnly, boolean crawl) {
    try {
      ...
      repository.updateCanaryRun(
          runId, summary.finishedAt(), runStatus, summaryJson, errorSummaryJson);
    } catch (Exception e) {
  ...
  private CrawlOutcome runAtsCanaryCrawl(List<String> tickers, Instant deadline) {
    ...
    CrawlRunRequest request =
        new CrawlRunRequest(
            tickers, tickers.size(), null, null, null, null, null, false, false, true, null);
  ...
  private void mergeDomainErrors(
      DomainResolutionResult domainResolution, Map<String, Integer> topErrors) {
  ...
  private record CrawlOutcome(
      long crawlRunId, int jobsExtracted, String status, String abortReason, long durationMs) {
