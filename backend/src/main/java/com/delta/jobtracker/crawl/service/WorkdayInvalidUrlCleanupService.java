...
public class WorkdayInvalidUrlCleanupService {
  ...
  public WorkdayInvalidUrlCleanupService(
      PoliteHttpClient httpClient, CrawlJdbcRepository repository) {
    this.httpClient = httpClient;
    this.repository = repository;
  }

  public WorkdayInvalidUrlCleanupResponse cleanupInvalidWorkdayUrls(
      Integer limit, Integer batchSize, boolean dryRun) {
    int safeLimit = limit == null || limit <= 0 ? Integer.MAX_VALUE : Math.max(1, limit);
    int safeBatchSize =
        batchSize == null || batchSize <= 0 ? DEFAULT_BATCH_SIZE : Math.max(1, batchSize);
    int scanned = 0;
    int invalid = 0;
    int deleted = 0;
    int errors = 0;
    ...
    return new WorkdayInvalidUrlCleanupResponse(
        scanned, invalid, deleted, errors, dryRun, lastId, limitReached);
  }
