...
public class CareersDiscoveryRunService {
    ...
    public CareersDiscoveryRunResponse startAsync(Integer limit, Integer batchSize, Boolean vendorProbeOnly) {
        ...
        discoveryExecutor.submit(() -> runDiscovery(runId, companies, safeBatchSize, vendorProbeOnlyMode));
    ...
    public CareersDiscoveryRunStatus getLatestRunStatus() {
        ...
        Map<String, Long> failuresByReason = repository.countCareersDiscoveryCompanyFailures(status.discoveryRunId());
        return withFailures(status, failuresByReason);
    }
    ...
    private void runDiscovery(long runId, List<CompanyTarget> companies, int batchSize, boolean vendorProbeOnly) {
        int processed = 0;
        int succeeded = 0;
        int failed = 0;
        ...
        CareersDiscoveryService.DiscoveryMetrics metrics = new CareersDiscoveryService.DiscoveryMetrics();
                ...
                for (CompanyTarget company : batch) {
                    ...
                    try {
                        if (company.domain() == null || company.domain().isBlank()) {
                            ...
                            CareersDiscoveryService.DiscoveryOutcome outcome = discoveryService.discoverForCompany(
                                company,
                                deadline,
                                metrics,
                                vendorProbeLimiter,
    ...
    private CareersDiscoveryService.VendorProbeLimiter buildVendorProbeLimiter() {
        ...
        Map<String, java.util.concurrent.atomic.AtomicInteger> counts = new java.util.concurrent.ConcurrentHashMap<>();
        return host -> {
            if (host == null || host.isBlank()) {
                return false;
            }
            java.util.concurrent.atomic.AtomicInteger counter = counts.computeIfAbsent(host, ignored -> new java.util.concurrent.atomic.AtomicInteger(0));
