...
class CrawlOrchestratorCloseoutSafetyTest {
  ...
  void doesNotCloseOutWhenCompanyCrawlUnsafe() {
    ...
    when(repository.findCompanyTargetsWithAts(any(), anyInt()))
        .thenReturn(List.of(new CompanyTarget(1L, "ABC", "ABC Inc", null, "abc.com", null)));
    when(companyCrawlerService.crawlCompany(eq(1L), any(), any()))
        .thenReturn(
            new CompanyCrawlSummary(
                1L, "ABC", "abc.com", 0, 0, List.of(), 0, 0, false, Map.of("crawl_failed", 1)));

    CrawlOrchestratorService service =
        new CrawlOrchestratorService(
            repository,
    ...
    CrawlRunRequest request =
        new CrawlRunRequest(List.of(), 1, null, null, null, null, null, false, false, null, null);
