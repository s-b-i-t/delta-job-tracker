...
class FrontierSchedulerServiceObservabilityTest {
  ...
  private FrontierSchedulerService service;
  ...
  @Test
  void logsPerFetchTimingBreakdown(CapturedOutput output) {
    ...
    when(frontierRepository.countDueUrlsBlockedByBackoff(FrontierUrlKind.SITEMAP))
        .thenAnswer(
            invocation -> {
              sleepForMillis(6);
              return 3;
    ...
    AtomicLong enqueueIds = new AtomicLong(500L);
        ...
        .thenAnswer(
            invocation -> {
              ...
              return new FrontierEnqueueResult(
                  true, enqueueIds.incrementAndGet(), url, "example.com");
            });
    doAnswer(
            invocation -> {
              sleepForMillis(7);
    ...
    FrontierSchedulerResult result = service.fetchDueSitemaps(2, "seed-test-run");

    assertEquals(1, result.urlsFetched());
    assertEquals(3, result.urlsEnqueued());
    assertThat(result.diagnostics().dbMillis()).isPositive();
    assertThat(result.diagnostics().downstreamMillis()).isPositive();
    assertThat(output)
        .contains("frontier.seed.scheduler.start runId=seed-test-run")
        .contains("frontier.seed.scheduler.fetch.begin runId=seed-test-run")
        .contains("frontier.seed.scheduler.robots.finish runId=seed-test-run")
        .contains("frontier.seed.scheduler.http.finish runId=seed-test-run")
        .contains("frontier.seed.scheduler.fetch.finish runId=seed-test-run")
        .contains("frontier.seed.scheduler.summary runId=seed-test-run")
        .contains("claimNextDueUrlDbMs=")
        .contains("httpFetchMs=")
        .contains("completeFetchDbMs=");
  }
