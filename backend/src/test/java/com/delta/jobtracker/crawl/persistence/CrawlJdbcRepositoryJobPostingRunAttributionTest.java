...
class CrawlJdbcRepositoryJobPostingRunAttributionTest {
  ...
  void upsertJobPostingUsesLastSeenCrawlRunAttribution() {
    ...
    long companyId =
        repository.upsertCompany("ZZ" + suffix, "Run Attribution Co " + suffix, "Technology");

    long firstRunId =
        repository.insertCrawlRun(Instant.now().minusSeconds(120), "RUNNING", "first run");
    long secondRunId =
        repository.insertCrawlRun(Instant.now().minusSeconds(60), "RUNNING", "second run");
