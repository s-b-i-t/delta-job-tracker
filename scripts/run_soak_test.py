...
def run_phase(
    ...
    while time.monotonic() < deadline:
        ...
        if phase_cfg.domain_fresh_cohort_first:
            cmd.append("--domain-fresh-cohort-first")
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--soak-resolve-limit", type=int, default=120)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ok = run_phase(
        repo_root, out_root, warmup, args.base_url, args.crawler_max_duration_seconds, cycles
    )
    write_report(out_root / "soak_report.json", started_at, None, cycles, "RUNNING")
    if ok:
        ok = run_phase(
            repo_root, out_root, soak, args.base_url, args.crawler_max_duration_seconds, cycles
        )
